...
public class Preventivo {
	...
	public void calcola(ModuloCt content){
		if (check(content)){
			Anno_Inizio = Integer.parseInt(Inizio.substring(0,3)); /* Prende i valori delle date inserite. */
			...
			Diff_Anno = Anno_Fine - Anno_Inizio; /* Calcola la differenza tra le date. */
			...
			try{
				...
				if(preventivo.rs.next()){
					...
					if (Diff_Anno==0 && (Diff_Mese==0 || Diff_Mese==1) && Diff_Giorno<=7){ /* Caso in cui il preventivo è per un noleggio da 1 a 7 giorni. */
						if (Diff_Giorno==1){
							Giorni="1_Giorno";
