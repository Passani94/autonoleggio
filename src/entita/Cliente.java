...
public class Cliente{
	...
	public void aggiungi(ModuloCliente content){
		if (check(content)){
			try{
				...
				/*Verifica se il cliente che si vuole aggiungere � gi� presente nel DB.*/
				if (cliente.rs.next()){
					JOptionPane.showMessageDialog(null, "Errore! Esiste gi� un cliente con tale CF/Partita IVA!",
							"Errore ",
	...
	public void elimina(ModuloCliente content){
		if (checkelimina(content)){
			try{
				cliente.exequery("SELECT * FROM cliente where CF_PIVA='"+CF_PIVA+"'","select");
				/*Verifica se � presente un cliente con tale CF/Partita IVA.*/
				if(!cliente.rs.next()){
					JOptionPane.showMessageDialog(null, "Errore! Non � presente un cliente con tale CF/Partita IVA!",
							"Errore ",
							JOptionPane.ERROR_MESSAGE);
					content.txtCF_PIVA.requestFocus();
				}else {
	...
	private boolean check(ModuloCliente content){
		...
		if (CF_PIVA.equals("") || content.comboBoxTipologia.getSelectedIndex()==0 || RS.equals("")){	
			...
			content.txtCF_PIVA.requestFocus();
			check=false;
			JOptionPane.showMessageDialog(null, "Errore! Il Codice Fiscale inserito non � valido!",
			    "Errore ",
			    JOptionPane.ERROR_MESSAGE);
			...
			check=false;
			JOptionPane.showMessageDialog(null, "Errore! Il campo Citt� deve avere meno di 20 caratteri!",
			    "Errore ",
			    JOptionPane.ERROR_MESSAGE);
...
/* Metodo. Verifica la correttezza del CF (o della Partita IVA) del cliente da eliminare. */
	...
	/* Metodo. Verifica se una stringa � numerica. */
